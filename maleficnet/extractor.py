...
from utils import bits_to_str, bits_from_bytes
...
class Extractor:
    ...
    def __init__(self, seed: int, device: str, malware_length: int, hash_length: int, chunk_factor: int):
        self.seed = seed
        self.device = device
        self.H = None
        self.G = None
    ...
    def extract(self, model, message_length, return_decoded=True):
        ...
        if self.CHUNK_SIZE * self.chunk_factor * number_of_chunks > len(models_w_curr):
            raise ValueError("f'Spreading codes cannot be bigger than the model!'")
